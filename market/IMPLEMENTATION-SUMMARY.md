...
为 Peanuts 项目的 market 模块添加了 **WebSocket 实时推送功能**，在交易发生时（`onTrade` 调用）自动将 Ticker 和 K线数据推送给已连接的客户端。
...
```
market/src/main/java/com/ganten/peanuts/market/
├── websocket/
...
```bash
python3 /Users/ganten/workspace/github/peanuts/market/test-websocket.py
```
...
| 文档 | 位置 | 内容 |
|------|------|------|
| **README-WEBSOCKET.md** | `market/` | 完整设计文档 (9 章节) |
| **QUICKSTART.sh** | `market/` | 快速开始脚本 |
| **websocket-client.html** | `resources/` | Web UI 源代码 |
| **test-websocket.py** | `market/` | Python 测试工具 |
...
✓ **已完成**: 为 market 添加了完整的 WebSocket 实时推送功能
